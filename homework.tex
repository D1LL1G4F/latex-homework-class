...
\begin{alphaparts}
    \item
    \textbf{Yes.} Becuase for every subset of 3 columns of the array every 3-tuple of elements appear in max 1 row.
    \item
    If $p$ is a prime and $d \geq 2$ is an integer then there exists an orthogonal array $OA(p,\frac{p^d-1}{p-1},p^{d-2})$ \\
    since $p = 2$ then $2 = 2^{d-2} \implies d = 3$ \\
    $\frac{2^3-1}{2-1} = 7$ \\
    $2$ is prime $\cap$ $ 3 \geq 2 \implies $OA$(2,7,2)$ \textbf{does exist} \\
    This does not hold for OA$(2,8,2)$ therefore it \textbf{does not exist}
\end{alphaparts}
\end{numedquestion}
